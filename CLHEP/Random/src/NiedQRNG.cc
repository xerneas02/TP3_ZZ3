...
//int NiedQRNG::numEngines = 0;

// Maximum index into the seed table
//int NiedQRNG::maxIndex = 215;

/*NiedQRNG::NiedQRNG(long seed)
{
  setSeed(seed,0);
  setSeeds(&theSeed,0);
}*/

NiedQRNG::NiedQRNG()     	// 15 Feb. 1998  JMM
{
  q = gsl_qrng_alloc (gsl_qrng_niederreiter_2, 1);
}

/*NiedQRNG::NiedQRNG(int rowIndex, int colIndex) // 15 Feb. 1998  JMM
{
  long seed;
   long seeds[2];
...
/*NiedQRNG::NiedQRNG(HepStd::istream& is)
{
  is >> *this;
}*/

NiedQRNG::~NiedQRNG() {gsl_qrng_free (q);}

/*NiedQRNG::NiedQRNG(const NiedQRNG &p)
{
  int ipos, jpos;
  if ((this != &p) && (&p)) {
    theSeed = p.getSeed();
...
/*NiedQRNG & NiedQRNG::operator = (const NiedQRNG &p)
{
  int ipos, jpos;
  if ((this != &p) && (&p)) {
    theSeed = p.getSeed();
...
/*void NiedQRNG::saveStatus( const char filename[] ) const
{
...
/*void NiedQRNG::restoreStatus( const char filename[] )
{
   int ipos, jpos;
...
/*void NiedQRNG::showStatus() const
{
   HepStd::cout << HepStd::endl;
   HepStd::cout << "----- NiedQRNG engine status -----" << HepStd::endl;
...
/*void NiedQRNG::setSeed(long seed, int)
{
...
/*void NiedQRNG::setSeeds(const long* seeds, int)
{
  setSeed(seeds ? *seeds : 19780503L, 0);
  theSeeds = seeds;
}*/

double NiedQRNG::flat()
{
   double uni[1];

   if(gsl_qrng_get (q, uni) == GSL_EFAILED){
     printf("Overflow Sobol\n");
     throw new Exception("Sobol Overflow");
  }
...
void NiedQRNG::flatArray(const int size, double* vect)
{
//   double uni;
   int i;
...
/*HepStd::istream & operator >> ( HepStd::istream& is, NiedQRNG& e )
{
  int ipos, jpos;
