 ...
 * @ingroup random
 */
class SobolQRNGB: public HepRandomEngine {

public:

  //SobolQRNG(HepStd::istream& is);
  SobolQRNGB(int dimension = 1, int inSizeOfBrew = 1024, HepRandomEngine * brewer = new HepJamesRandom());
  //SobolQRNG(long seed);
 ...
 void setSeed(long seed, int dum=0){std::cout << "setSeed" << std::endl;throw new NotImplementedException("Methode non implementee");};
  ...
  virtual std::string name() const {return "SobolQRNGB";}
...
private:
  ...
  Sobol * q;
  HepRandomEngine * b;
  std::vector< double * > * brewingTable;

  //double * brewingTable;
  int indexOfCurrentVector;
  int sizeOfBrew;
  int dimension;
  int nextValueIndex;
