 ...
 * @ingroup random
 */
class Unrolled: public HepRandomEngine {

public:

  //SobolQRNG(HepStd::istream& is);
  Unrolled(const String & UnrolledFilleName);
  //SobolQRNG(long seed);
 ...
 void setSeed(long seed, int dum=0){std::cout << "setSeed" << std::endl;throw new NotImplementedException("Methode non implementee");};
  ...
  void saveStatus( const char filename[] = "Unrolled.conf" ) const{std::cout << "saveStatus" << std::endl;throw new NotImplementedException("Methode non implementee");};
  // Saves on file JamesRand.conf the current engine status.

  void restoreStatus( const char filename[] = "Unrolled.conf" ){std::cout << "restoreStatus" << std::endl;throw new NotImplementedException("Methode non implementee");};
  ...
  virtual std::string name() const {return "Unrolled";}
